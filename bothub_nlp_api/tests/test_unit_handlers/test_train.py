...
from bothub_nlp_api.utils import AuthorizationIsRequired, ValidationError
...
class TestTrainHandler(unittest.TestCase):
    ...
    def test_invalid_language(self):
        with self.assertRaises(ValidationError):
            train_handler(
                self.authorization, self.repository_version, "invalid_language"
            )
        with self.assertRaises(ValidationError):
    ...
    @patch("bothub_nlp_api.handlers.train.send_job_train_ai_platform", return_value={})
    @patch(
    ...
    def test_specific_language(self, *args):
        ...
        expected = {
            "SUPPORTED_LANGUAGES": ["pt_br", "en"],
            "languages_report": {"pt_br": {"status": "processing"}},
        }
        self.assertEqual(r, expected)
