...
class TestEvaluateHandler(unittest.TestCase):
    ...
    @patch(
        "bothub_backend.bothub.BothubBackend.request_backend_evaluate", return_value={}
    )
    ...
    def test_automatic_evaluate_invalid_model(self, *args):
        with self.assertRaises(ValidationError):
            crossvalidation_evaluate_handler(
                self.authorization, "en", self.repository_version
            )
    ...
    @patch(
        "bothub_nlp_api.handlers.evaluate.send_job_train_ai_platform", return_value={}
    )
    @patch(
