...
class TestDebugParseRoute(unittest.TestCase):
    ...
    def test_v1_invalid_authorization(self):
        ...
        response = self.app.post("debug_parse", headers=invalid_header)
    ...
    def test_v1_invalid_text(self):
        ...
        response = self.app.post(
            "debug_parse", headers=self.header, data=invalid_body
        )
        self.assertEqual(422, response.status_code)
    ...
    def test_v1_debug_parse(self, *args):
        body = {"text": "test", "language": "en"}
        response = self.app.post("debug_parse", headers=self.header, data=body)
